...
public class StudentRecordStoreTest
{
    ...
    public StudentRecordStoreTest()
    {
        ...
        services.AddStore<StudentRecordStore, Student>();
    ...
    public void SubscriptionWillBeCalledAfterActionGetInvoked()
    {
        ...
        var action = Substitute.For<Action<Student>>();
        store.Subscribe(action);
        store.UpdateName("Ahmed");
        action.Received(1).Invoke(Arg.Is<Student>(s => s.Name == "Ahmed"));
        store.GetState().Should().BeEquivalentTo(new Student("Ahmed", 21));
    }
    ...
    public void ValidationShouldBeCalledAndPreventStateUpdateInCaseOfInvalidValue()
    {
        ...
        action.Received(0);
        store.GetState().Should().BeEquivalentTo(new Student("John", 21));
